...
export const ListItem = observer(({ taskId, content, isDone }: IListItemProps) => {
    ...
    return (
        <ListItemDIV>
            <ListItemToggle onClick={handleSelectTask} $isDone={isDone}></ListItemToggle>
            <TodoContent $isDone={isDone}>{content}</TodoContent>
            <ListItemDeleteButton onClick={handleDeleteTask}></ListItemDeleteButton>
        </ListItemDIV>
    );
});
